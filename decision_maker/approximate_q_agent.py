...
class ApproxQAgent(InfiniteRTAgent):
    ...
    def train_subproblem_builder_fn(self, env, scenario_id, init_state = None):
        ...
        # Split the objective into the action-INDEPENDENT cost part (kept as a
        # fixed expression) and the per-period penalty terms (re-applied each
        # iteration for the current coefficients). `penalty_terms` stores the
        # (state_var, action_var, new_arrival, likelihood_ratio) used by each
        # period's penalty so both the objective rebuild and the feature-vector
        # cut can be evaluated later.
        ...
        for period_index, new_arrival in enumerate(self.delta[scenario_id]):
            ...
            penalty_terms.append((state_var, action_var, new_arrival, likelihood_ratio))
            ...
            cost_part = cost_part + likelihood_ratio * one_time_cost

        def objective_builder_fn(model, action_values):
            """Set the LP objective = cost_part + sum_t lr_t * penalty_t(action).

            ``action_values`` is the master coefficient vector. With the
            coefficients fixed to it, every penalty term is LINEAR in the
            decisions, so the resulting objective is linear (genuine LP)."""
            coefficients = np.asarray(action_values, dtype=float).reshape(-1)
            coefficient_blocks = generating_function.get_coefficients(coefficients)
            penalty = gp.quicksum(
                lr * generating_function.calculate_penalty(
                    sv, av, na, is_var=True, coefficients=coefficient_blocks)
                for (sv, av, na, lr) in penalty_terms
            )
            model.setObjective(cost_part + penalty, GRB.MINIMIZE)

        def cut_gradient_fn(model, action_values):
            """Return the Benders subgradient phi(x*) = d ObjVal / d action.

            phi_k(x*) = sum_t lr_t * grad_t(x*)_k, evaluated at the optimal
            primal solution. This equals the dual of the old
            ``coefficients == action`` linking constraint, so the generated cut
            matches the previous (QP) formulation."""
            duals = np.zeros(number_of_coefficients, dtype=float)
            for (sv, av, na, lr) in penalty_terms:
                state_val = tuple(np.asarray(component.X, dtype=float) for component in sv)
                action_val = (np.asarray(av[0].X, dtype=float),
                              np.asarray(av[1].X, dtype=float))
                grad = np.asarray(
                    generating_function.calculate_gradient(
                        state_val, action_val, na, is_var=False),
                    dtype=float,
                )
                duals += lr * grad
            return duals
    ...
    def benders_decomposition_train(self,
        ...
        upper_bound, info = self.coefficient_model.solve(init_solution=init_solution, is_hard_bound=True, max_iter=1500, parallel=False, verbose=verbose, checkpoint_path=checkpoint_path, resume_checkpoint_path=resume_checkpoint_path)
