...
class ApproxQAgent(InfiniteRTAgent):
    ...
    def train_subproblem_builder_fn(self, env, scenario_id, init_state = None):
        ...
        # Cold solve uses barrier (Method=2) + crossover, which is much faster
        # than dual simplex from scratch on the large ejor-scale LPs. After the
        # first solve we switch to Method=1 so subsequent Benders iterations
        # re-optimize from the saved basis with dual simplex in a few pivots.
        sub_model.setParam("Method", 2)
        # Force crossover so the cold barrier solve produces a usable simplex
        # basis for later warm starts (Gurobi can otherwise skip it on very
        # large LPs and leave only an interior point).
        sub_model.setParam("Crossover", 1)
        # LPWarmStart=2 keeps both the simplex basis AND the presolved LP across
        # successive optimize() calls on the same model object, so Gurobi only
        # repeats the work made invalid by the RHS change. This is the key
        # speedup for repeated subproblem solves in Benders.
        ...
        generating_function = self._require_generating_function()
        coefficient_vars = generating_function.get_coefficient_var(model=sub_model, coefficient_bound=GRB.INFINITY)
        # Local coefficient blocks (no shared-state mutation) -> thread-safe build.
        coefficient_blocks = generating_function.get_coefficients(coefficient_vars)
        coefficient_linking_constraints = generating_function.build_coefficient_linking_constraints(sub_model, coefficient_vars)
        ...
        for period_index, new_arrival in enumerate(self.delta[scenario_id]):
            ...
            penalty = generating_function.calculate_penalty(state_var, action_var, new_arrival, is_var=True, coefficients=coefficient_blocks)
        ...
        # Pre-solve once at build time with barrier+crossover so each worker
        # carries an initial basis into the Benders loop. The build-time RHS
        # corresponds to all-zero penalty coefficients, which is exactly the
        # state the master starts from in iteration 1, so the basis is a
        # near-perfect warm start. Both this solve AND the construction above
        # run in parallel across env groups via _build_training_workers.
        sub_model.Params.OutputFlag = 0
        sub_model.optimize()
        # All later optimize() calls only differ in linking-constraint RHS; dual
        # simplex with LPWarmStart=2 re-optimizes from the basis above.
    ...
    def _build_training_workers(self, init_state=None, parallel=True, verbose=False):
        ...
        def build_one(sid):
            ...
            worker = SubproblemWorker(
                ...
                verbose=verbose, grb_env=envs[sid])
