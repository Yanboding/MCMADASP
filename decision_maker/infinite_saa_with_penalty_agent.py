...
class InfinitePenalizedSAAAgent(InfiniteRTAgent):
    ...
    def master_builder_fn(self):
        ...
        # add action constraint
        self.add_action_space_constraints(model=model, state_var=state_var, action_var=action_var)
        eta_var = model.addVar(vtype=GRB.CONTINUOUS, lb=-GRB.INFINITY, name="eta")
        imm_cost = self.env.cost_fn(state_var, action_var, is_var=True)
        model.setObjective(imm_cost + eta_var, GRB.MINIMIZE)
        return model, imm_cost, eta_var, action_var, state_linking_constraints
    
    def second_stage_builder_fn(self, coefficient_bound=GRB.INFINITY):
        master_model = gp.Model(f"Second_Master", env=self.grb_env)
    ...
    def benders_decomposition_solve(self, state, action=None,
        ...
        if self.bender_solver is None:
            ...
            workers = {}
            for omega in range(self.sample_path_number):
                sub_model, action_linking_constraints, state_linking_constraints = self.subproblem_builder_fn(env=acquire_grb_env(), scenario_id=omega)
                workers[omega] = SubproblemWorker(sub_model=sub_model,
                                                  action_linking_constraints=action_linking_constraints,
                                                  state_linking_constraints=state_linking_constraints)
            self.bender_solver = BendersDecompositionSolver(master_model=master_model,
                                                            workers=workers,
                                                            imm_cost=imm_cost,
