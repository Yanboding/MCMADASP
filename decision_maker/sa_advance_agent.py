...
class SAAdvanceAgent:
    ...
    def solve(self, state, t, x, num_sample_paths):
        ...
        for omega in range(num_sample_paths):
            new_arrivals = self.env.reset_arrivals(t + 1)
        ...
        for tau in range(1, N - t + 1):
            a_t_tau_omega = model.addVars(M, N - t - tau + 1, I, vtype=GRB.INTEGER, name="a_t_j_tau_omega")
        ...
        y_future = model.addVars(M, range(1, N - t + 1), lb=0, vtype=GRB.CONTINUOUS, name="y_future")
        # immediate_cost: sum_{i,j} w_i [ b^t_{j,i} + a^t_{j,i} ]
        immediate_cost = gp.quicksum(w[i] * gp.quicksum(b[j, i] + a_t[j, i] for j in range(N-t+1))
        ...
        #                + O * y^{t+tau}_ω
        #              ]
        future_cost = gp.quicksum(gp.quicksum(gamma ** tau * (gp.quicksum(w[i] * gp.quicksum(b[tau+j, i] + a_t[tau+j, i]
        ...
        for tau in range(1, N - t + 1):
            for i in range(I):
                for omega in range(M):
                    model.addConstr(gp.quicksum(a_future[tau][omega, j, i] for j in range(N - t - tau + 1)) ==  delta[omega, tau-1, i], name=f"cons4_delta_{tau}_{i}_om{omega}")
        ...
        model.addConstr(
            y_t >= gp.quicksum((b[0, i] + a_t[0, i]) * r[0, i] for i in range(I)) - C,
            name="cons5"
        )
        #
        # (cons7) and (cons8):
        ...
        for tau in range(1, N - t + 1):
            for omega in range(M):
                model.addConstr(
                    y_future[omega, tau] >= gp.quicksum((b[tau, i] + a_t[tau, i] + gp.quicksum(a_future[k][omega, tau-k, i] for k in range(1, tau+1))) * r[0, i] for i in range(I)) - C,
    ...
    def policy(self, state, t):
        action, overtime, obj_value = self.solve(state, t, None, 3000)
        return action
...
if __name__ =="__main__":
    ...
    arrival_generator = MultiClassPoissonArrivalGenerator(100, 3, [1 / class_number] * class_number)
    env_params = {
    ...
    print(state)
    solution_a, y_t_solution, obj_value = agent.solve(state, 1, None, 3000)
    print(solution_a)
    print(obj_value)
    # [[5, 0], [0, 5], [0, 1]]
