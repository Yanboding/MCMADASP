...
def ejor_default_case_config():
    ...
    return custom_case_config(**env_args)

def small_case_config():
...
def custom_case_config(
    ...
    return ExperimentConfig(env, reset_params, init_state, valid_action, args)

def get_config_by_type(case_type, args=None):
    if args is None:
        args = {}
    if case_type == 'ejor':
        config = ejor_base_case_config()
    elif case_type == 'ejor_default':
        config = ejor_default_case_config()
    elif case_type == 'small':
        config = small_case_config()
    elif case_type == 'toy':
        config = toy_case_config()
    elif case_type == 'infinite_custom':
        config = custom_case_config(**args)
    return config
